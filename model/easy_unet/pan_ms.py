...
class Multi_branch_Unet(nn.Module):
    ...
    def forward_2(self, x_t, t_input, cond, feat_need=False):  # 修改模型输入内容，修改为MS,PAN,NOISE三个部分，不能切片输入
        ...
        fd.append(h)
        # 编码器部分有PAN,MS的三层，解码器部分有五层
        if feat_need:
            return fe, Reverse(fd)
        else:
            return h

    def forward(self, x_t):  # 修改模型输入内容，修改为MS,PAN,NOISE三个部分，不能切片输入
        # Obtain the Gaussian random feature embedding for t
        cond = torch.randn((1, 1, 256, 256)).cuda()
