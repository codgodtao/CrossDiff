...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised=True, x_in=None, w=3.0):
        ...
        if x_in is not None:
            x_recon = self.predict_start_from_noise(  # 根据噪声得到x0
                x, t=t, noise=self.classifier_free_guidance_sample_pan2ms
    ...
    def get_feats(self, x_in, t):
        return None

    # def p_losses(self, x_in, noise=None):
    #     # 训练过程使用的L1损失函数
    #     x_start = x_in['HR']  # 这是参考数据残差 4*256*256
    #     [b, c, h, w] = x_start.shape
    #     t = np.random.randint(1, self.num_timesteps + 1)  # 随机选取一个timestamp索引
    ...
    #     x_noisy = self.q_sample(  # 对X0进行加噪得到xt
    #         x_start=x_start, continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1), noise=noise)
    #
    #     time_in = torch.tensor([t] * b, device=x_start.device).view(b, -1)
    #     assert time_in.shape == continuous_sqrt_alpha_cumprod.shape
    #
    #     if not self.conditional:
    #         x_recon = self.denoise_fn(x_noisy, time_in)
    #     else:
    #         condition_x = self.classifier_free_guidance_train(x_in, p_uncond=0.2)
    #         x_recon = self.denoise_fn(
    #             torch.cat([condition_x, x_noisy], dim=1), time_in)  # 传入上采样的MS和噪声信息
    #
    ...
    def classifier_free_guidance_train(self, x_in, p_uncond):
        ...
        #     return condition
        # else:
        #     return torch.zeros_like(x_in['MS'])
        rand = torch.rand(1)
        PAN = x_in['PAN']
        if rand > p_uncond:
            # condition_x = torch.cat([x_in['MS'], x_in['PAN']], dim=1)
