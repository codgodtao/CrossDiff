...
from ..unsupervised_metric.spectral_tools import gen_mtf
...
def local_cross_correlation(img_1, img_2, half_width):
    ...
    img_1 = np.pad(img_1.astype(np.float64), ((w, w), (w, w), (0, 0)))  # w=2 ,在img_1上下左右填充2个pixel，最后是256*256
...
def local_cross_correlation_torch(img_1, img_2, half_width):
    """
        Cross-Correlation Field computation.
    ...
    """
    w = int(half_width)
    ep = torch.tensor(1e-20).to(img_1.device, dtype=img_1.dtype)
    if len(img_1.shape) != 3:
    ...
    img_1 = torch.nn.functional.pad(img_1, (0, 0, w, w, w, w), mode='constant', value=0)
    img_2 = torch.nn.functional.pad(img_2, (0, 0, w, w, w, w), mode='constant', value=0)  # 是否拓展为260？
    # img_1 = torch.pad(img_1.to(torch.float64), (w, w, w, w, 0, 0))
    # img_2 = torch.pad(img_2.to(torch.float64), (w, w, w, w, 0, 0))

    img_1_cum = torch.zeros_like(img_1)
    img_2_cum = torch.zeros_like(img_2)
    for i in range(img_1.shape[-1]):
        # integral(img_1[:, :, i]).astype(np.float64)
        img_1_cum[:, :, i] = torch.cumsum(img_1[:, :, i], dim=0).to(torch.float64)
    for i in range(img_2.shape[-1]):
        img_2_cum[:, :, i] = torch.cumsum(img_2[:, :, i], dim=0).to(torch.float64)
    ...
    # img_2 = torch.pad(img_2.to(torch.float64), (w, w, w, w, 0, 0))
    img_1 = torch.nn.functional.pad(img_1, (0, 0, w, w, w, w), mode='constant', value=0)
    img_2 = torch.nn.functional.pad(img_2, (0, 0, w, w, w, w), mode='constant', value=0)

    i2 = img_1 ** 2
    j2 = img_2 ** 2
    ij = img_1 * img_2

    i2_cum = torch.zeros_like(i2)
    j2_cum = torch.zeros_like(j2)
    ij_cum = torch.zeros_like(ij)

    for i in range(i2.shape[-1]):
        i2_cum[:, :, i] = torch.cumsum(i2[:, :, i], dim=0).to(torch.float64)
    for i in range(j2.shape[-1]):
        j2_cum[:, :, i] = torch.cumsum(j2[:, :, i], dim=0).to(torch.float64)
    for i in range(ij.shape[-1]):
        ij_cum[:, :, i] = torch.cumsum(ij[:, :, i], dim=0).to(torch.float64)
    ...
    sig2_ii_tot = torch.clamp(sig2_ii_tot, ep, sig2_ii_tot.max())
    sig2_jj_tot = torch.clamp(sig2_jj_tot, ep, sig2_jj_tot.max())

    xcorr = sig2_ij_tot / (torch.sqrt(sig2_ii_tot * sig2_jj_tot) + ep)
...
def SAM(outputs, labels):
    ...
    angle = np.nansum(np.arccos(np.clip(scalar_product / norm_product, a_min=-1, a_max=1)), axis=-1) / \
...
def SAM_torch(outputs, labels):
    """
        ...
        outputs : torch.Tensor
            The Fused image. Dimensions: N, C, H, W
        labels : torch.Tensor
            The reference image. Dimensions: N, C, H, W
    ...
    norm_outputs = torch.sum(outputs ** 2, dim=1)
    norm_labels = torch.sum(labels ** 2, dim=1)
    ...
    scalar_product.masked_fill_(mask, float('nan'))
    norm_product.masked_fill_(mask, float('nan'))

    scalar_product = scalar_product.view(-1)
    norm_product = norm_product.view(-1)

    cosine_similarity = scalar_product / norm_product
    cosine_similarity = torch.clamp(cosine_similarity, min=-1, max=1)

    angle_in_radian = torch.acos(cosine_similarity)
    angle_in_radian = torch.nan_to_num(angle_in_radian, nan=0)

    angle_in_degree = torch.mean(angle_in_radian) * (180 / np.pi)

    return angle_in_degree.item()
...
def DRho_torch(outputs, pan, sigma=4):
    half_width = ceil(sigma / 2)
    rho = torch.clip(local_cross_correlation_torch(outputs, pan, half_width), min=-1.0, max=1.0)
    d_rho = 1.0 - rho
    return torch.mean(d_rho)


if __name__ == '__main__':
    gt = torch.ones([256, 256, 4]) * 255
    ms = torch.randn([256, 256, 4]) * 255
    gt_numpy = gt.numpy()
    ms_numpy = ms.numpy()
    sam = SAM(gt_numpy, ms_numpy)
    gt = torch.ones([1, 4, 256, 256]) * 255
    ms = torch.randn([1, 4, 256, 256]) * 255
    sam2 = SAM_torch(gt,ms)
    print(sam2,sam)
    # re = DRho(gt_numpy, pan_numpy, 4)
    # result = DRho_torch(gt, pan, 4)
    # print(re, result)
